...
**重要**: GitHub issueは問題の定義と期待される結果の明確化に焦点を当てます。具体的な作業手順やタスク分割は`plan-issue`スキルで別途作成されるため、issue本文には含めません。
...
## 技術的な考慮事項（必要に応じて）
[技術的な制約や選択肢の概要のみ記載。具体的な実装手順は記載しない]
...
- 機能提案: 「期待される効果」を詳しく、実装の詳細は避ける

**注意事項**:
- 具体的な作業手順やファイル単位のタスク分割は記載しない
- これらは`plan-issue`スキルで別途作成される
- issue本文は「何を」「なぜ」に焦点を当て、「どのように（詳細）」は避ける
