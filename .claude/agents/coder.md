...
2. **テストとLintのスコープ**: テストとLintを実行する際は、実装している特定のファイルのみを対象にします。makeコマンドではなく、Goの直接コマンドを使用して単一ファイルを指定します：
   - Lint: `golangci-lint run <file>.go`
   - Test: `go test -run <TestFunctionName>` または特定のテストファイルのパッケージのみを対象
...
3. 完全なテストコードを表示
4. 実行したLintとテストコマンド（`golangci-lint run`と`go test -run`）をリスト化
5. すべての品質チェックの結果を報告
...
- [ ] すべてのテストが単独で実行時に合格する（`go test -run`で確認）
