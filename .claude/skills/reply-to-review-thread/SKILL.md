...
description: PRレビュースレッドに返信を投稿します。
---

## 手順

このスキルは、指定したレビュースレッドに返信コメントを投稿します。
コメント投稿者への@メンションが自動的に追加され、標準入力からコメント本文を読み取ります。
...
1. 未解決コメントを確認: `get-pr-review-comments` スキル
2. スレッドの詳細を確認: `get-pr-review-thread-details` スキル
3. 指摘事項を修正してコミット・プッシュ
4. **このスキルで返信を投稿**（修正内容の説明、質問への回答など）
5. 必要に応じてスレッドを解決: `resolve-pr-thread` スキル
